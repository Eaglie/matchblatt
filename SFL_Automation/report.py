...
/* SPIELER-CONTAINER: KOMPLETT TRANSPARENT */
.player{
    position:absolute;
    transform:translate(-50%,-50%);
    display:flex;
...
def draw_players(spieler):
    ...
    for p in spieler or []:
        ...
        html_parts.append(
            f'''
<div class="player"
     style="left:{html.escape(str(left))}%;
            top:{html.escape(str(top))}%">

    <div class="player_circle">
        {html.escape(str(nummer))}
    </div>
...
<tr>
    <td>Gesperrt</td>
    <td>{values("gesperrt")}</td>
</tr>

<tr>
    <td>Verletzt</td>
    <td>{values("verletzt")}</td>
</tr>
...
<div class="header">
    ...
    <div class="title">
        ...
        <h1>
            {html.escape(str(heim_name)).upper()}
            –
            {html.escape(str(gast_name)).upper()}
        </h1>
